...
| 住所信頼度バッジ（ドライバー画面） | UI 統合は次フェーズ（API・型定義は実装済み） |
| 入口/建物/表札メモ 入力 UI | 表示のみ実装・入力フォームは次フェーズ |
| 配送表画像 private Blob 化 | 現状は public URL・次フェーズで認証経由に変更予定 |
...
| 機能 | 状態 |
|---|---|
| 入口/建物/表札/駐車/注意メモ 入力フォーム | ⬜ 未実装 |
| 管理画面予測バッジ表示（取込確認・ルート） | ⬜ 未実装 |
| field_status_json MANUAL_FIXED 書き込み（手修正時） | ⬜ 未実装 |

**フェーズ5: 画像管理**

| 機能 | 状態 |
|---|---|
| 配送表画像 private Blob 化 | ⬜ 未実装 |
