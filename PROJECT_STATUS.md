...
### 🆕 クルー配送カードに削除機能（2026-07-04）
取込ミス/重複配送をクルー自身が消せる。`DELETE /api/driver/delivery-items/[id]`（本人担当は明細/割当/フォローごと削除、フォロー中のみはフォロー解除）。カードヘッダーにゴミ箱→確認バー（やめる/削除する）。本番で動作確認済（誤読ダミー削除OK）。
