...
> 最終更新: 2026-06-30（本番テスト中・OCR精度改善検討中）
...
| エンジン | 状態 | 問題 |
|---|---|---|
| Tesseract.js | ❌ 廃止 | Vercel 10秒タイムアウト超過 |
| OCR.space（現在） | ⚠️ 精度不足 | 日本語テーブルの住所・数量が混入・誤認識 |
| Cloud Vision | ⏳ 未設定 | 精度◎・月1,000回無料だが課金登録が必要 |
| **Gemini API** | 🔄 検討中 | **精度◎◎・1,500回/日無料・カード不要** |
...
- 氏名・電話番号：△ 部分的に読み取り
- 住所：❌ 他フィールドのテキストが混入
- 数量（常温/クーラー/ケース/総数）：❌ 誤認識多数

### 解決策候補

#### 推奨：Gemini API に切り替え

- Google AI Studio で無料 API キー取得（クレカ不要）
- 1,500リクエスト/日 = 月45,000回 → 運用量に余裕
- テーブル構造を理解して JSON で返せる → パーサー不要
- https://aistudio.google.com/apikey

#### 代替：Cloud Vision + 課金設定

- 月1,000回無料、超過分 $1.50/千件（ドライバー10名×6枚×25日=1,500枚で超過）
- 課金登録が必要

### 次のアクション

`GEMINI_API_KEY` を取得したら Gemini OCR に切り替え実装を行う。
...
| 2026-06-30 | ⚠️ OCR精度問題 | OCR.space の日本語テーブル精度不足（住所混入・数量誤認識）→ Gemini API 切り替えを検討中 |
