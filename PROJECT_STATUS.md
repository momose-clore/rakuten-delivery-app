...
> 最終更新: 2026-06-30（住所補正・手動ピン修正・配送メモ 実装完了）
...
| 機能 | ステータス |
|---|---|
| `delivery_location_overrides` テーブル | ✅ スキーマ定義済み・✅ Neon migration 適用済み・⬜ UI 未実装（次フェーズ） |
| 手動ピン修正 UI | ✅ 管理画面で承認・却下フロー実装済み |
| 住所信頼度表示 | ✅ API・型定義実装済み（UI統合は次フェーズ） |
| 入口/建物/表札メモ | ⬜ 次フェーズ |
...
| 機能 | 理由 |
|---|---|
| `src/lib/address/` | ✅ 住所正規化・信頼度・オーバーライドマッチャー実装済み |
| `src/lib/maps/googleMapsUrl.ts` | `src/lib/maps/url.ts` が担当（既存） |
| `/admin/location-overrides` | ✅ 住所補正管理画面（一覧・承認・却下）実装済み |
| `/admin/import-accuracy` | 取込精度レポートUI（次フェーズ） |
| `src/lib/import/spreadsheet/` | `src/lib/import/csv/` に統合済み |
| 住所信頼度表示 | 次フェーズ |
| 入口/建物/表札/駐車メモ UI | 次フェーズ |
| 過去配送履歴の再利用 | 次フェーズ |
| 管理者承認フロー | 次フェーズ |
| 配送表画像 private Blob 化 | 次フェーズ（現状は public URL） |
...
| API キー | `OCR_SPACE_API_KEY`（未設定時はデモキー `helloworld`） |
...
| # | 内容 |
|---|---|
| 1 | `OCR_SPACE_API_KEY` が未設定の場合はデモキーで動作（1日500回・1MB以下制限） |
