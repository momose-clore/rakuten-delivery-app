...
### 追記（2026-07-13・その6）：本番反映完了＋カウントのサイト内編集
- **本番デプロイ**：`9b225d7` 成功（`5cbbc59` は一度Vercelビルド失敗→再デプロイで解消。ローカルbuildは常時OK＝一時要因）。
- **本番へ7月データ投入**：`import-line` に **トークン認証(EXTRA_VEHICLE_PULL_TOKEN)＋from フィルタ** を追加し、LINEエクスポートを本番へ投入（`from=2026-07-01`＝6月除外）。**HTTP200・7/1〜7/12・222台** 反映確認。
- **カウントのサイト内編集**：月次グリッドの全セル（貼付/SP/増車）を number 入力で直接編集可能に。貼付/増車は自動集計への**手動上書き**（`vehicle_count_manual` にカテゴリ別保存・上書きは黄色表示・clearで自動へ復帰）。SPは従来通り手入力。
- API拡張：`POST /api/admin/vehicle-count { date, waveNo, category(貼付|SP|増車), count, clear? }`（後方互換 sp 維持）。`getVehicleCountProgress` が override 適用＋`overrides`返却。
- 実DB検証：自動4→手入力9→clearで4復帰。typecheck/lint/build ✅。
...
- **【α→γ 緊急対応済 6aef585】** γコミットの `src/lib/ocr/barcode.ts`（zxing-wasmで直接デコード＝良い方針）が **本番ビルドを全滅**させていた（`sharp.Sharp/Metadata`名前空間型＋`readBarcodes`引数型）。**型のみ最小修正で復旧済**（ロジック不変・未配線で実行時影響なし）。γは `readBarcodes` の実入力型（zxing-wasm v3=ImageData/Blob）を確認して調整を。**方針: バーコードは「zxing直接デコード(γ・高精度)」＋「OCR.space印字数字(α・barcodeText)」の二段構え**推奨（zxing失敗時にbarcodeTextでフォールバック）。
...
**追加修復（2026-07-09・依頼者GO／本番デプロイ Ready 済）**:
- **M2 Medium**: 本番で Postgres 接続に `sslmode=require` を強制（`src/lib/prisma.ts` `ensureSsl`）。
- **H4 High（部分緩和）**: ログインにメール単位のインメモリ・レート制限（10分/8回失敗で拒否）。※サーバーレスは分散制限に `@upstash/ratelimit`（無料枠・要アカウント）or Vercel WAF が別途必要。OCR/upload/geocode のコスト濫用対策は未適用。
- **H5 High**: `xlsx@0.18.5`（CVE-2023-30533 / CVE-2024-22363）を SheetJS 公式CDNのパッチ版 **0.20.3** へ差替（`npm audit` の high を 0 に低減）。Excel取込のAPI互換・typecheck/lint/build 確認済。γ の台数管理表もこの版を使用可。
- **M1 Medium**: `err.message` 返却4箇所は型付き `CarioApiError` の安全文言のみ（キー/URL/個人情報を含まない）＝実質漏れなしを検証、追加変更不要と判断。

**残（依頼者判断・要外部設定）**: H4 の分散レート制限＋OCR/upload/geocode（Upstash等）/ M3 next-auth 安定版追随 / M4 postcss（Next 16.x パッチ追随）/ L2 画像 Content-Disposition（nosniff済・実害低）。詳細は `docs/security-status.json`。
...
### 手動四隅調整＋射影変換（デワープ）実装（2026-07-04）
斜め撮り対策。自動書類検出は雑然背景で不安定（sharp/OpenCV実証NG）のため**手動四隅指定**を主に（CamScanner方式）。
- `src/lib/image/perspective.ts`：ホモグラフィ解法（純関数・node検証で四隅誤差0）
- `CornerAdjuster.tsx`：撮影後に4隅をドラッグ→ブラウザ内canvasでバイリニア射影変換→アップロード（作業2200px上限）
- 撮影フロー：capture→**adjust(四隅調整)**→quality→OCR。「補正せずそのまま取込」＋ワープ失敗時は原本にフォールバック（既存フロー非破壊）
- 前処理：unsharp化で4000px画像も1MB以下（ペイロード保護での解像度低下を回避）
- **要 riku 実機テスト**（カメラ/タッチはα環境で検証不可）。barcodeText(左上バーコード)はOCR.spaceで取得済、DB保存はschema調整待ち(γ)。

### 実写真(IMG_7705.HEIC)検証と結論（2026-07-04）
riku提供の実カメラ写真で根因を特定：
- **HEIC変換失敗**：iPhone HEICはサーバー `sharp(libheif)` がセキュリティ制限(iref>16)で復号不可→生HEICがOCRへ→崩れる。**対策: ブラウザ側でHEIC→JPEG正規化(`to-jpeg.ts`, `normalizeToJpegBlob`)してから送信**（Safari復号可・失敗時は原本）。
- **写真が90°横向き＋小写り＋反射**：切り出して正立させても、OCR.spaceが密な小文字を**順不同**に読み住所が散乱（伝票No/氏名/ラベル混入・逆順）。＝**スマホ生写真は当帳票のOCR限界**。
- **結論／運用推奨**：**PDF/CamScannerスキャンは満点**。密なL1M帳票は「document-scannerで撮る（CamScanner / iOS内蔵スキャン）→PDF/クリーン画像を取込」が確実。アプリ内カメラ生写真は補助（四隅補正で改善するが実スキャナには及ばず）。撮影ガイドを「画面いっぱい/縦向き正面/反射回避」に強化済。
...
### 🔧 依頼（β→γ）: src/lib/ocr/barcode.ts の型エラー修正
> βの作業（管理画面・アカウント）とは無関係。全体 typecheck が赤のため、担当のγ（commit f931975・OCRバーコード）へ修正依頼。βは当該ファイルを触りません。

`npm run typecheck` で以下が発生（2026-07-04 時点）:
- `barcode.ts(40,13) TS2503: Cannot find namespace 'sharp'`（`sharp.Metadata` を型として使用）
- `barcode.ts(50,31) TS2503: Cannot find namespace 'sharp'`（`sharp.Sharp` を型として使用）
- `barcode.ts(61,38) TS2345: ImageData に colorSpace が無い`（`{data,width,height}` を ImageData 引数へ渡している）

想定修正（担当判断で）:
- 型は `import sharp, { type Metadata, type Sharp } from "sharp"` などにして `sharp.Metadata`→`Metadata` / `sharp.Sharp`→`Sharp` に。
