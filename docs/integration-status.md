...
- [ ] **A① 2-opt/Or-opt を追加**（純コード・外部API不要）… `src/lib/routes/*`。まずこれだけでも順序改善。
...
- [ ] **D② 住所正規化を変換前に強化**（全角半角/表記ゆれ/建物名除去→ROOFTOPヒット率↑。OCRの正規化資産流用）。
- [ ] **D③ 精度ランクで自動フラグ**（`locationType` が APPROXIMATE/GEOMETRIC_CENTER のものを「⚠要確認ピン」に自動化→補正導線へ）… location-override 連携。
- [ ] **D④ 承認済みピン(override)の同一住所 自動再利用**（直すほど精度↑）。
- [ ] **D⑤ GSI 無料フォールバック/検算**（Google失敗時・低精度時に突き合わせ、大きくズレたら自動フラグ）。
...
## 📷 バーコード直接デコード（γ→α 共同・実動改善）2026-07-04
