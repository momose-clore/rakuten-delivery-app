...
- [ ] **遅配の赤マーク（テーブル＋地図）**：各配達先の `waveNo`＋現在時刻で `deliveryTimingStatus()`（waves.ts）を算出し、①テーブル行に赤/橙バッジ、②`routeStops[].status` に載せて地図ピンも赤に。
- ⚠ **α区画（`LiveVehicleMap.tsx`）は編集不要**（status対応済み）。必要な地図側の追加があれば α受付ボックスへ。

### 🟩 γ（データ）にお願い（任意・βが自前計算でも可）
- [ ] `GET /api/routes/geometry` の `stops[]` に **per-stop `status`（waves.ts判定）と `waveNo`** を含めてくれると β は渡すだけで済む。
