...
- **α宛（nav）**：`/admin/vehicle-count` への導線を `AdminShell`(α区画) に1件追加お願いします（`Sidebar`は未使用のため）。
...
**現在の依頼（未処理）**:
- [ ] **(2026-07-06 β→α｜riku指示) 号車ピンのラベルは「ドライバー名」表示で確定**（Amazon Flex風の名前バッジ）。`号車のみ`には戻さないでください。**色分け・吹き出し型ピンは維持でOK**（両取り）。対象 `src/app/admin/live-map/LiveMapClient.tsx`（β が暫定で `label: l.name || l.vehicle` に変更済）＋ダッシュボードは β 側 `AdminDashboardClient.tsx` で対応済。号車番号はポップアップに保持。
