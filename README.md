...
| `GOOGLE_CLOUD_VISION_API_KEY` | OCR 実行用（STEP 3 以降必須） |
...
| 種別 | メール | パスワード |
|---|---|---|
| 管理者 | admin@delivery-app.local | admin1234 |
| ドライバー① | tanaka@delivery-app.local | driver1234 |
| ドライバー② | sato@delivery-app.local | driver1234 |
| ドライバー③ | suzuki@delivery-app.local | driver1234 |
...
| 4 | OCR API キー | 設定欄のみ | `GOOGLE_CLOUD_VISION_API_KEY` を本番キーに設定 |
...
| `GOOGLE_CLOUD_VISION_API_KEY` | Google Cloud Console で発行 | ✅ |
...
| `admin1234 / driver1234` パスワード | `ADMIN_PASSWORD` 環境変数で本番パスワードを設定 |
